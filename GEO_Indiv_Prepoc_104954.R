...
filePaths <- getGEOSuppFiles(exper, baseDir = paste0(wd,'/CKD_Data/GSE104954/')

    
#Extract CEL files from TAR file
    
if(file.exists(file.path(wd, 'Documents', 'CKD_Data', 'GSE104954', exper, paste0(exper, '_RAW')))) {
  untar(rownames(filePaths)[1], exdir = file.path(wd, 'Documents', 'CKD_Data', 'GSE104954', exper, paste0(exper,'_RAW')))
} else {
  dir.create(file.path(wd, 'CKD_Data', 'GSE104954', exper,paste0(exper, '_RAW')))
  untar(rownames(filePaths)[1], exdir = file.path(wd, 'Documents', 'CKD_Data', 'GSE104954', exper, paste0(exper,'_RAW')))}
